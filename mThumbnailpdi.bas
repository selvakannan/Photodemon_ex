Attribute VB_Name = "mThumbnailpdi"
Option Explicit
Option Compare Text
...
Public Const IMAGETYPES_MASKPDI As String = "|XCF|PSD|ORA|PSP|PDF|SVG|PDI|WEBP|TGA|ICO|JNG|KOALA|LBM|IFF|MNG|PBM|PCD|PCX|PGM|PPM|RAS|WBMP|CUT|XPM|XBM|DDS|HDR|FAXG3|SGI|EXR|J2K|JP2|PFM|PICT|RAW|JXR|PNM|HEIF|MBM|CBZ|AVIF|JLS|QOI|HGT|JXL|PNG|JPG|JPEG|BMP|GIF|WMF|EMF|TIF|TIFF|"

Private Type DATABASE_INFO
    Size    As Long
    Entries As Long
End Type
...
Public Sub UpdateFolder(ByVal sFolder As String)
  ...
  Dim aData() As Byte
    ...
    With fMainpdi.ucThumbnailView
        
        '-- Get files list
        If (pvGetFiles(IMAGETYPES_MASKPDI, uFile())) Then
  ...
  m_CRC.Algorithm = Crc32
       ...
       strhex = Hex(m_CRC.CalculateFile(m_sFolder & uFile(lItem).Filename))


                Call .ItemAdd(lItem, uFile(lItem).Filename, uFile(lItem).FileSize, uFile(lItem).FileDate, strhex)
            Next lItem
            ...
            '-- Add/Get thumbnails
            fMainpdi.ucProgress.Max = UBound(uFile()) + 1
            
            For lItem = 0 To .Count - 1
                
                '-- Current progress
                fMainpdi.ucProgress.Value = lItem + 1
...
Public Sub UpdateItem(ByVal sPath As String, ByVal lItem As Long)
  ...
  Dim hSearch As Long
    ...
    m_sFolder = sPath
    hSearch = FindFirstFile(m_sFolder & fMainpdi.ucThumbnailView.ItemText(lItem, [tvFileName]) & vbNullChar, uWFD)
    ...
    If (hSearch <> INVALID_HANDLE_VALUE) Then
        ...
        If (Not m_oRecordset.NoMatch) Then
            Call m_oRecordset.Edit
            Call mThumbnailpdi.pvSetThumbnail(uFile, lItem)
            Call m_oRecordset.Update
            '-- Refresh item
            Call fMainpdi.ucThumbnailView.RefreshItems(lItem, lItem)
        End If
    End If
...
Private Sub pvSetThumbnail(ByRef uFile As FILE_INFO, ByVal lItem As Long)
   ...
   '------------------------------------------
   sPath = m_sFolder & uFile.Filename '  Call SendMessage(m_hDlg, CDM_GETFILEPATH, MAX_PATH, ByVal sPath)
   ...
   If (sPath <> vbNullString) Then
     ...
     End If
        ...
        tmpFilename = UserPrefs.GetTempPath & "\" & "untitled.png"

       fileExtension = UCase$(Files.FileGetExtension(sPath))
    'Depending on the file's extension, load the image using the most appropriate image decoding routine
    Select Case fileExtension
    
           Case "PSD", "PDI", "PDF", "PCX", "PCD", "ORA", "MBM", "KOA", "HDP", "JXR", "JXL", "JLS", "HGT", "HEIF", "HEIC", "G3", "DNG", "CBZ", "AVIF", "PFM", "PGM", "PIC", "PICT", "APNG", "PNM", "PPM", "PSP", "QOI", "RAW", "RGB", "BW", "SVG", "WEBP", "XCF", "XPM", "JNG", "KOALA", "LBM", "IFF", "LBM", "MNG", "PBM", "PBMRAW", "PPMRAW", "RAS", "TARGA", "TGA", "WBMP", "CUT", "XBM", "DDS", "GIF", "HDR", "FAXG3", "SGI", "EXR", "J2K", "JP2", "PNG", "JPG", "JPEG", "BMP", "GIF", "WMF", "EMF", "ICO", "TIF", "TIFF"
     ...
     ' tmpDIB.WriteToBitmapFile dstFilename
    ...
    Set tmpDIB = Nothing
     ...
     If (mGDIplus.GdipLoadImageFromFile(StrConv(tmpFilename, vbUnicode), hImage) = [Ok]) Then
        ...
        Call oDIBThumb.GetBestFitInfo(W, H, fMainpdi.ucThumbnailView.ThumbnailWidth, fMainpdi.ucThumbnailView.ThumbnailHeight, bfx, bfy, bfW, bfH)
    ...
    Call fMainpdi.ucThumbnailView.ThumbnailInfo_SetTBIH(lItem, aTBIH())
    Call fMainpdi.ucThumbnailView.ThumbnailInfo_SetData(lItem, aData())
     Kill tmpFilename
